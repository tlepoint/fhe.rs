...
/// Evaluation key for the BFV encryption scheme at a specific level.
...
impl LeveledEvaluationKeyBuilder {
	/// Creates a new builder form the [`SecretKey`] for operations at a given
	/// level.
	pub fn new(
		sk: &SecretKey,
		ciphertext_level: usize,
	...
	pub fn build(&mut self) -> Result<LeveledEvaluationKey> {
		let mut ek = LeveledEvaluationKey {
			...
			rot_to_gk_exponent: LeveledEvaluationKey::construct_rot_to_gk_exponent(&self.sk.par),
			monomials: vec![],
...
#[cfg(test)]
mod tests {
	use super::LeveledEvaluationKeyBuilder;
	...
	fn test_builder() -> Result<(), Box<dyn Error>> {
		...
		let mut builder = LeveledEvaluationKeyBuilder::new(&sk, 0, 0)?;
	...
	fn test_inner_sum() -> Result<(), Box<dyn Error>> {
		for params in [Arc::new(BfvParameters::default(2))] {
			for _ in 0..25 {
				...
				assert_eq!(
					Vec::<u64>::try_decode(&pt, Encoding::Simd(0))?,
		...
		for params in [
			...
			for _ in 0..20 {
				for i in 1..1 + log_degree as usize {
					...
					let pt = Plaintext::try_encode(&v as &[u64], Encoding::Poly(0), &params)?;
					let ct = sk.try_encrypt(&pt)?;
					...
					for (vi, ct2i) in izip!(&v, &ct2) {
						...
						assert_eq!(expected, Vec::<u64>::try_decode(&pt, Encoding::Poly(0))?);
