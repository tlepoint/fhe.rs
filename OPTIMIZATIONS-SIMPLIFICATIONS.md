...
## Scalar dot products and compact NTT database storage

The preceding NTT-row reuse optimization was committed as `ceef063`. This
investigation targets the scalar ciphertext/plaintext dot product used by both
PIR examples, with 1,000,000 records of 288 bytes and release builds throughout.

### Result retained

The existing uncompressed scalar FMA remains the fastest of the tested
variants. Added an optional memory-oriented path instead:

- `PackedPlaintext` stores each canonical NTT residue in the bit width of its
  ciphertext modulus; `unpack()` restores the original plaintext without an
  NTT, preserving its encoding, level, and local timing permission.
- `PackedPlaintextVec` packs rows into one contiguous allocation. Its borrowed
  views feed `DotProductScalarWorkspace::dot_product_scalar_packed` directly.
  This avoids both individual allocator size-class padding and constructing a
  full unpacked NTT database during preprocessing.
- The new kernel fuses unpacking with 64-by-64-bit multiplication into the
  existing u128 accumulator. It specializes the eight possible bit offsets,
  rather than particular PIR moduli, and also supports residues that straddle
  nine bytes. It allocates no unpacked plaintext buffer and performs no NTTs
  during a dot product.
- Canonical-input overflow bounds are unchanged. Long packed dot products
  reduce periodically, reserving room for the previous residue. The schedule
  depends only on public moduli and operand count. Variable-time reductions
  require every operand to permit them. Scratch is cleared on return and
  unwinding; packed buffers are cleared on drop.
- Packing writes whole words instead of one byte at a time. Both PIR examples
  expose `--packed-database`; the ordinary NTT database remains the default.

Use either example with the additional flag:

```sh
cargo run --example mulpir --release -- --database-size 1000000 --element-size 288 --packed-database
cargo run --example sealpir --release -- --database-size 1000000 --element-size 288 --packed-database
```

The library API also accepts individual packed plaintexts. For a database,
prefer `PackedPlaintextVec::with_capacity(&params, level, rows)`, append each
encoded plaintext using `push`, then pass `packed.iter()` (or a strided column)
to the workspace method. Borrowed views do not retain plaintext encoding
metadata; standalone `PackedPlaintext` objects do.

Both examples print the raw record size and, after preprocessing, the encoded
database size with its `packed NTT` or `unpacked NTT` format. The encoded figure
reports coefficient storage (including packing padding), not whole-process
memory or allocator overhead.

This is exact storage compression: it changes neither ciphertext coefficients
nor the noise/rounding schedule, encryption parameters, or wire formats. The
initial database NTT still has to be computed. Ordinary BFV multiplication,
relinearization, and decryption do not use the new path automatically.

### Time and memory measurements

The final focused `pir_dot_product` benchmark uses independently generated,
dense plaintexts, the real 174-by-81 and 447-by-64 shapes, and full database
scans in addition to a single-column control. Ten flat samples are collected
per case. This avoids relying on the mostly zero records generated by the
examples. Times below are Criterion point estimates on this Apple M2 with the
default NTT backend, not guarantees for other machines.

| Work | MulPIR | SealPIR |
|---|---:|---:|
| Ordinary column | 2.712 ms | 3.595 ms |
| Packed column (individual objects) | 3.824 ms | 3.976 ms |
| Re-encode coefficient data, then compute column | 26.599 ms | 27.683 ms |
| Ordinary full matrix | 225.91 ms | 240.31 ms |
| Packed full matrix (contiguous storage) | 260.31 ms | 259.01 ms |

Thus the packed scan costs about 15.2% more for MulPIR and 7.8% more for SealPIR.
The complete response also includes expansion and other BFV operations, so
its increase is smaller. The final single-column reconstruction measurements
were noisy (MulPIR's 95% interval was 24.684–28.968 ms; SealPIR's was
25.300–32.219 ms), so their ratios indicate the size of the tradeoff rather
than a precise speed estimate. Earlier passes measured roughly 7–9 times the
ordinary column cost. Full-matrix measurements were considerably steadier.

| Encoded NTT coefficient payload | MulPIR | SealPIR |
|---|---:|---:|
| Ordinary u64 storage | 1,847,328,768 bytes | 1,874,853,888 bytes |
| Contiguous packed storage, including padding | 1,515,386,888 bytes | 1,054,605,320 bytes |
| Reduction | 18.0% | 43.75% |

MulPIR uses 50+55 bits per coefficient instead of 128; SealPIR uses 36+36.
These are payload sizes, excluding the raw database, ciphertexts, keys,
metadata, and allocator overhead. Whole-process peak memory is measured
separately using `wait4`'s maximum resident set size, in bytes on macOS.

Four alternating pairs per example, with five server responses per invocation,
produced these medians. Every invocation verified the returned record bytes.

| Whole example | MulPIR ordinary → packed | SealPIR ordinary → packed |
|---|---:|---:|
| Server response | 613.40 → 641.10 ms (+4.5%) | 498.40 → 509.45 ms (+2.2%) |
| Peak resident memory | 2.314 → 1.973 GB (−14.7%) | 2.360 → 1.532 GB (−35.1%) |
| Database preprocessing | 1987.50 → 2296.55 ms | 1853.25 → 2064.65 ms |
| Entire process wall time | 5.323 → 6.119 s | 4.589 → 5.055 s |

The full command is slower: preprocessing and destruction are included in its
wall time. This mode is useful when database memory matters and preprocessing
can be amortized over queries; it is not a throughput improvement.

Individual packed objects initially saved less resident memory because the
allocator rounded their allocations up. Contiguous storage removed that cost.
The scalar accumulator itself retains its existing size (512 KiB for MulPIR,
256 KiB for SealPIR); the large saving comes from the database.

### Can we omit the plaintext NTT?

Keeping coefficient values as u32 would reduce their payload by 75% relative
to the two-row u64 NTT database. The PIR example's original 20-bit packing can
be smaller still. But reconstructing the encoded plaintexts on each query
measured 9.8 times the ordinary column cost for MulPIR and 7.7 times for
SealPIR, including allocation and conversion as well as the transforms.
This coefficient-only path was measured in the focused benchmark, not added
as an example option.

The current operation is a sum of pointwise products of transformed
ciphertext and plaintext polynomials. Expanded ciphertexts are dense even
when their decrypted selection values are sparse. Removing the plaintext
transform from that expression requires polynomial convolutions or another
way of computing those transforms; linearity alone does not remove the
multiplication work. Recomputing the NTTs through the current encoding API
does not offer similar response time. Packing their exact residues does.

### Other scalar-kernel experiments

The original ARM assembly already uses paired loads/stores with `mul`,
`umulh`, `adds`, and `adc`. The following prototypes were measured and removed:

| Prototype | Observed result relative to the ordinary matrix scan |
|---|---|
| 512-coefficient tiles, 16 KiB accumulator | About twice as slow; extra visits and scattered input access outweighed the smaller scratch |
| One RNS row at a time, half the accumulator | About 9–12% slower |
| Four operands per FMA pass | MulPIR about 3% slower; SealPIR essentially unchanged |
| ARM vector multiplication of bounded small limbs | About 39–47% slower after splitting and accumulator traffic |
| Sharing each plaintext coefficient across both ciphertext parts | About 9–14% slower |

A simple unpack-then-dot implementation was also roughly three times the
ordinary scan cost. Fusing bit extraction into multiplication is necessary
for the retained memory/time tradeoff. Sharing unpacking between two
ciphertext parts was tested too and lost performance despite fewer extractions.

A further speed-oriented candidate is processing several database columns
together to reuse the expanded-query ciphertexts, which are larger than the
cache. That is a matrix traversal change; it was not implemented or measured
in this investigation.

### Validation and reproducibility

Added tests for every packing/decoding width from 1 through 64, maximal
residues and random data, the final padded word, independently computed scalar
products, periodic reduction boundaries, multiple levels and ciphertext part
counts, timing permissions, contiguous-buffer growth, error recovery,
zeroization, and iterator-panic cleanup. The fused decoder also checks buffer
bounds in release mode before entering its unchecked inner loop. Example tests compare packed and
ordinary database processing while retaining every-byte and padding checks.
...
cargo test -p fhe --release --all-features packed
cargo test -p fhe --example mulpir --example sealpir
RUSTDOCFLAGS='-D warnings' cargo doc -p fhe --no-deps
cargo bench -p fhe --bench pir_dot_product -- --save-baseline scalar-final
```

Local measurement artifacts: `/tmp/fhe-dot-final-bench.log`,
`/tmp/fhe-dot-final-paired-results.json`, and
`/tmp/fhe-dot-final-{mulpir,sealpir}-{dense,packed}-{0,1,2,3}.log`.
