...
impl RnsContext {
	...
	pub fn new(moduli: &[u64]) -> std::option::Option<Self> {
		if moduli.is_empty() {
			None
		} else {
			...
			for i in 0..moduli.len() {
				...
				for j in 0..moduli.len() {
					if i != j {
						let (d, _, _) =
							BigUint::from(moduli[i]).extended_gcd(&BigUint::from(moduli[j]));
	...
	pub fn project(&self, a: &BigUint) -> Vec<u64> {
		let mut rests = Vec::with_capacity(self.moduli.len());
...
mod tests {
	...
	pub fn test_constructor() {
		...
		assert!(RnsContext::new(&[1, 2]).is_some());
